...
export async function POST(request: Request) {
  try {
    const body = (await request.json()) as SendReminderBody;

    const { to, staffName, clinicName, unreadProcedures, dashboardUrl } = body;

    if (!to || !staffName || !clinicName || !dashboardUrl) {
      return NextResponse.json(
        { error: "Missing required fields: to, staffName, clinicName, dashboardUrl" },
        { status: 400 }
      );
    }

    if (!Array.isArray(unreadProcedures) || unreadProcedures.length === 0) {
      return NextResponse.json(
        { error: "unreadProcedures must be a non-empty array" },
        { status: 400 }
      );
    }
